...
FrameCallback frames[] = { drawDateTime, drawEnergy1, drawEnergy2, drawTemp };
...
//  display.setCursor(0,0);
  ...
  display.print(Irms1*230);   
  display.println(" W");

  display.print("POWER2: ");
  display.print(Irms2*230); 
  display.println(" W");
...
void drawEnergy1(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->setFont(ArialMT_Plain_10);
  display->drawString(64 + x, 5 + y, "Power Watt1");
  display->setFont(ArialMT_Plain_24);
  //calcVI(20,100); 
  value = Irms1*230;
...
void drawEnergy2(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->setFont(ArialMT_Plain_10);
  display->drawString(64 + x, 5 + y, "Power Watt2");
  display->setFont(ArialMT_Plain_24);
  //calcVI(20,100); 
  value = Irms2*230;
  dtostrf (value, 3, 1, message_buff);
...
void drawTemp(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  display->setFont(ArialMT_Plain_10);
  display->drawString(64 + x, 5 + y, "Temperature");
  display->setFont(ArialMT_Plain_24);
  dtostrf (temp, 3, 1, message_buff);
  String strValue = String(message_buff); 
  display->drawString(64 + x, 15 + y, strValue + "°C");
}
